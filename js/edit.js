...
function editWalk(walkID) {
    ...
    $('#edit_walk_right_button').on('click', function(e) {
        e.preventDefault();
        var confirm_date = $('#edit_walk_date').val();
        var walk_date = confirm_date.split('/');
        var formatted_walk_date = walk_date[2] + '-' + walk_date[0] + '-' + walk_date[1] + ' ';
        var confirm_hour = $('#edit_walk_hour').val();
        var walk_hour = parseInt(confirm_hour);
